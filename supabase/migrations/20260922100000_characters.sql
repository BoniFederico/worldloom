...
create table public.campaign_stats (
  ...
  -- Numero di revisione: cresce a ogni salvataggio dello schema (predisposizione per la migrazione guidata, #35).
...
begin
  -- Chi non fa parte della campagna non deve poter distinguere nulla dai messaggi che seguono: rifiuto immediato.
  if not private.can_manage_campaign(new.campaign_id) and private.campaign_role(new.campaign_id) is null then
  ...
  -- Il proprietario si controlla quando è nuovo o cambia: se poi il giocatore esce dalla campagna, il DM può ancora modificare la scheda.
    ...
    loop
      ...
      if v_old is distinct from v_new then
        v_changes := v_changes || jsonb_build_object(
          v_section || '.' || v_key,
          -- Solo numeri (o assenza) si copiano: un valore qualunque arrivato dall'API non gonfia la cronologia.
          case when v_section in ('attributes', 'resources')
                 and coalesce(jsonb_typeof(v_old), 'number') = 'number' and coalesce(jsonb_typeof(v_new), 'number') = 'number'
            then jsonb_build_array(v_old, v_new) else 'true'::jsonb end
        );
      end if;
    end loop;
  end loop;
