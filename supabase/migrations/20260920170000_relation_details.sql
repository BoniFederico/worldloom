...
language sql immutable set search_path = '' as $$
  ...
  select v is null or coalesce((
    ...
    and (v ->> 'year') ~ '^-?[0-9]{1,9}$'
    and (v -> 'month' is null or ((v ->> 'month') ~ '^[0-9]{1,2}$' and (v ->> 'month')::int between 1 and 99))
    and (v -> 'day' is null or ((v ->> 'day') ~ '^[0-9]{1,2}$' and (v ->> 'day')::int between 1 and 99))
...
alter table public.relations
  ...
  add constraint relations_valid_time check (private.valid_time(valid_from) and private.valid_time(valid_to)),
  add constraint relations_valid_order check (
    valid_from is null or valid_to is null
    or (valid_from ->> 'year')::bigint <= (valid_to ->> 'year')::bigint
  );
